...
int main()
{
	...
	//let's assume we have breakpoint at pos4 then

	//let's test
	int leftRange = 4;
	int travelRange = length - leftRange;
	//most difficult to derive
	additionalTimefornode1 = travelRange + (travelRange - (nodepos1-leftRange));

	for (int time = 0; time < 22; time++)
	{
		int val1 = leftRange+pingpong(time + additionalTimefornode1, travelRange);
	}
	//ok goes well
