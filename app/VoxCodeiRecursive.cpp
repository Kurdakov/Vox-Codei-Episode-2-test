...
class GameField {
public:
	GameField(int _width, int _height) :width(_width), height(_height), possibles({})
	{
	...
	bool simulate(int frame, int& bombs, nodesdesque_t& nodes, deque<shared_ptr<Possibility>>& seq)
	{
		...
		if (frame >= rounds)
		{
			return false;//last round achived
		}
		...
		for (shared_ptr<Node> pNode : nodes)
		{
		...
		deque<shared_ptr<Possibility>>::iterator iter = seq.begin();
		while (iter != seq.end())
		{
